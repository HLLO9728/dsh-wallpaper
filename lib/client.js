...
`
    ...
    /** Append a media element without clearing existing ones (for cross-fades). */
    function makeMediaAppend(el, entry) {
      const media = document.createElement(entry.type === 'video' ? 'video' : 'img')
      media.setAttribute('data-dsh-wallpaper-media', '')
      media.alt = entry.title || ''
    ...
    /**
     * Wait until a media element actually has a decodable frame, so the new
     * wallpaper is never shown as an empty black frame.
     */
    function whenReady(media, resolve) {
      if (media.readyState >= 2 || media.complete === true) {
        resolve(); return
      }
      const ok = () => resolve()
      media.addEventListener('load', ok, { once: true })
      media.addEventListener('loadeddata', ok, { once: true })
      media.addEventListener('canplay', ok, { once: true })
      const t = setTimeout(ok, 2500)
      media._wpReadyCleanup = () => clearTimeout(t)
    }
    ...
    function render(root, cfg) {
      ...
      // Carousel mode: cross-fade through every wallpaper at intervalSec.
      ...
      let idx = 0
      let current = null
      // First wallpaper fades in once it has a frame (avoids an initial black flash).
      {
        const first = makeMediaAppend(stage, list[0])
        first.style.opacity = '0'
        first.style.transition = 'none'
        whenReady(first, () => {
          if (first.parentNode !== stage) return
          first.style.transition = transition
          first.style.opacity = stageOpacity
          current = first
        })
      }
      ...
      const stop = () => {
        clearInterval(timer)
        for (const m of stage.querySelectorAll('[data-dsh-wallpaper-media]')) m._wpReadyCleanup && m._wpReadyCleanup()
      }

      const timer = setInterval(() => {
        idx = (idx + 1) % list.length
        const next = makeMediaAppend(stage, list[idx])
        next.style.opacity = '0'
        next.style.transition = 'none'
        const prev = current
        // Fade the previous one out as the new one fades in, after the new
        // one actually has a frame — so there is never a black gap.
        whenReady(next, () => {
          ...
          if (prev && prev !== next) {
            ...
            prev._wpReadyCleanup = () => clearTimeout(t)
          }
          current = next
        })
      }, intervalMs)
