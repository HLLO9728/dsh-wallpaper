...
> 几个上百 MB 的视频（尤其同时轮播）会显著占用内存和 CPU，可能导致切换卡顿、黑屏甚至页面卡死。
> 建议：每个视频尽量在 **几十 MB 以内**，并适当调大 `intervalSec`（例如 15 秒以上），
> 或改用图片/动图。卡顿时可先把 `enabled: false` 关掉壁纸立即恢复。
> 插件采用**单视频元素轮换**，不会叠加元素，能极大缓解多视频播放的资源占用。
...
- **让壁纸“穿透”显示**：插件会自动把 DSH 主界面的**最底层全幅表面**置为透明
  ...
  置为 `transparent`）。聊天输入框、气泡等表面也会被置为半透明以露出壁纸；再配合
  `dim`/`opacity` 让文字清晰。若某些版本布局变了导致壁纸被遮挡，可微调
  `lib/client.js` 里 `reveal` 的扫描逻辑。
- **轮播切换为淡入淡出**（不黑屏）：等新一帧就绪后再替换，过渡连贯。
- **轮播间隔**最小为 3 秒；`wallpaper.yml` 校验失败（如 YAML 写错）时不会崩溃，
  而是维持上一个可用配置。
